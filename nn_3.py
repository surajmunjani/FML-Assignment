...
class Net(object):
    ...
    def __init__(self, NUM_FEATS, num_layers, num_units):
        '''
...
def main():
    ...
    corr_matrix_list = train_input.corr().to_numpy().tolist()

    discarding_features = []
    threshold = 0.8
    for i in range(len(corr_matrix_list)):
        for j in range(i):
            if abs(corr_matrix_list[i][j]) > threshold:
    #         if corr_matrix_list[i][j] > threshold:
                discarding_features.append(j)
    
    NUM_FEATS = 90 - len(set(discarding_features))
    input_features = [ 'TimbreAvg'+str(i+1) if i<12 else 'TimbreCovariance'+str(i-11) for i in range(90)]
    for f in set(discarding_features):
        if f<12:
            input_features.remove('TimbreAvg'+str(f+1))
        else:
            input_features.remove('TimbreCovariance'+str(f-11))
    ...
    train_input_selected = train_input.drop([str(l+2) for l in set(discarding_features)],axis=1)
    dev_input_selected = dev_input.drop([str(l+2) for l in set(discarding_features)],axis=1)
    test_input_selected = test_input.drop([str(l+2) for l in set(discarding_features)],axis=1)

    train_input_normalized = feature_scaling(train_input_selected, method='z_score_normalization')
    dev_input_normalized = feature_scaling(dev_input_selected, method='z_score_normalization')
    test_input_normalized = feature_scaling(test_input_selected, method='z_score_normalization')

    net = Net(NUM_FEATS, num_layers, num_units)
...
#     get_test_data_predictions(net, test_input)
    pred = get_test_data_predictions(net,test_input_selected,relu)
